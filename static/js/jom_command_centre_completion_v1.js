(function(){
  ...
  function href(a){const k=key(a);const s=String(a.source||a.category||'').toLowerCase();if(k==='discovered-sites'||s.includes('estate_lifecycle')||s.includes('estate_admin_site_inventory'))return'/estate#discovered-sites';if(k==='admin-truth'||s.includes('admin_truth'))return'/reference';return(s.includes('runtime')||s.includes('operator'))?'/operator/observability':'/estate';}
  function label(a){const k=key(a);const s=String(a.source||a.category||'').toLowerCase();if(k==='discovered-sites'||s.includes('estate_lifecycle')||s.includes('estate_admin_site_inventory'))return'Open Estate review';if(k==='admin-truth'||s.includes('admin_truth'))return'Open Admin';return(s.includes('runtime')||s.includes('operator'))?'Open Runtime Status':'Open workspace';}
