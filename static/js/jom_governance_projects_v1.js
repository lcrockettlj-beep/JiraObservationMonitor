(() => {
  ...
  async function loadLeads(){const status=$("gp-lead-status"),body=$("gp-lead-rows");try{const response=await fetch("/api/governance/projects/leads",{headers:{Accept:"application/json"}});const data=await response.json();if(!response.ok||data.status!=="ok")throw new Error(data.reason||"Project Lead authority unavailable.");const rows=Array.isArray(data.projects)?data.projects:[];body.replaceChildren();rows.forEach(item=>{const tr=document.createElement("tr");const names=(item.leads||[]).map(x=>x.display_name).join(", ")||"Unavailable";[item.site_key,item.project_key,item.project_name,names,item.lead_state].forEach(v=>tr.appendChild(cell(v)));body.appendChild(tr);});$("gp-lead-count").textContent=`${data.summary.projects_with_supported_active_lead||0}/${data.summary.projects||0} supported`;status.textContent=`${data.authority_status} authority. Missing leads remain unavailable, not zero.`;status.classList.add("gp__status--ok");}catch(error){body.replaceChildren();$("gp-lead-count").textContent="Unavailable";status.textContent=error.message;status.classList.add("gp__status--blocked");}}
  async function loadOwners(){const status=$("gp-owner-status"),body=$("gp-owner-rows");try{const response=await fetch("/api/governance/projects/owners",{headers:{Accept:"application/json"}});const data=await response.json();if(!response.ok||data.status!=="ok")throw new Error(data.reason||"Space Owner authority unavailable.");const rows=Array.isArray(data.projects)?data.projects:[];body.replaceChildren();rows.forEach(item=>{const tr=document.createElement("tr");const names=(item.owners||[]).map(x=>x.display_name).join(", ")||"Unavailable";[item.site_key,item.project_key,item.project_name,names,item.owner_state].forEach(v=>tr.appendChild(cell(v)));body.appendChild(tr);});$("gp-owner-count").textContent=`${data.summary.projects_with_governance_defined_owner||0}/${data.summary.projects||0} owned`;status.textContent=`${data.authority_status} governance-derived authority. Project Lead is the GLI space owner; Jira has no native owner field.`;status.classList.add("gp__status--ok");}catch(error){body.replaceChildren();$("gp-owner-count").textContent="Unavailable";status.textContent=error.message;status.classList.add("gp__status--blocked");}}
  fields.forEach(f => $("gp-"+f).addEventListener(f==="search"?"input":"change",render)); load(); loadIdentities(); loadLeads(); loadOwners();
})();
