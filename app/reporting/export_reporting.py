...
def _report_value(report, path, default='-'):
    cur = report
...
def _site_table(sites):
    if not sites:
        return "<p class='jom-report-muted'>No site records were available in the registry snapshot.</p>"
    rows = []
    for site in sites:
        rows.append(
            '<tr>'
            f"<td>{_html_escape(site.get('display_name') or site.get('site_key'))}</td>"
            f"<td>{_html_escape(site.get('url'))}</td>"
            f"<td><span class='jom-report-pill'>{_html_escape(site.get('state'))}</span></td>"
            f"<td>{'Yes' if site.get('monitored') else 'No'}</td>"
            f"<td>{_html_escape(site.get('source'))}</td>"
            '</tr>'
        )
    return """
    <div class='jom-report-table-wrap'>
      <table class='jom-report-table'>
        <thead><tr><th>Site</th><th>URL</th><th>State</th><th>Monitored</th><th>Source</th></tr></thead>
...
def to_html(report: Dict[str, Any]) -> str:
    ...
    if kind == 'executive_summary':
        body = f"""
        <section class='jom-report-hero'>
          <p class='jom-report-kicker'>Executive Report</p>
          <h1>Executive Summary</h1>
          <p>High-level estate position, active risk and recommended management attention.</p>
        </section>
        ...
        <section class='jom-report-section'><h2>Recommended actions</h2>{_findings(['Review active alerts before sharing stakeholder outputs.', 'Review discovered sites and decide whether to monitor, reject, or keep pending.', 'Use Estate and Site Workspace for operational investigation.'])}</section>
        """
    elif kind == 'estate_summary':
        summary = report.get('summary') if isinstance(report.get('summary'), dict) else {}
        body = f"""
        <section class='jom-report-hero'>
          <p class='jom-report-kicker'>Estate Report</p>
          <h1>Estate Summary</h1>
          <p>Known Atlassian site estate, monitoring state and registry position.</p>
        </section>
        <section class='jom-report-grid'>
        ...
        """
    elif kind == 'governance_summary':
        users = report.get('users') if isinstance(report.get('users'), dict) else {}
        registry = report.get('registry') if isinstance(report.get('registry'), dict) else {}
        runtime = report.get('runtime') if isinstance(report.get('runtime'), dict) else {}
        product = report.get('product_access') if isinstance(report.get('product_access'), dict) else {}
        body = f"""
        <section class='jom-report-hero'>
          <p class='jom-report-kicker'>Governance Report</p>
          <h1>Governance Summary</h1>
          <p>Identity, access, source reliability and governance coverage snapshot.</p>
        </section>
        <section class='jom-report-grid'>
        ...
        body = f"""
        <section class='jom-report-hero'>
          <p class='jom-report-kicker'>Site Report</p>
          <h1>{_html_escape(title)}</h1>
          <p>Single-site operational snapshot.</p>
        </section>
        <section class='jom-report-grid'>
          {_report_card('Site key', report.get('site_key', '-'), 'Requested site')}
          {_report_card('Active alerts', report.get('active_alerts', '-'), 'Alert signal')}
          {_report_card('Runtime', _report_value(report, 'runtime.runtime_status'), 'Runtime state')}
        </section>
        """
...
<head>
  ...
  <style>
    :root{{--r-blue:#0c66e4;--r-ink:#092957;--r-muted:#44546f;--r-border:#dfe1e6;--r-bg:#f4f5f7}}
    body{{font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;background:var(--r-bg);color:var(--r-ink);margin:0;padding:32px}}
    main{{max-width:1120px;margin:auto;background:#fff;border:1px solid var(--r-border);border-radius:18px;padding:30px;box-shadow:0 18px 42px rgba(9,30,66,.12)}}
    h1{{font-size:38px;line-height:1.05;margin:8px 0 10px}} h2{{margin-top:0}} p{{color:var(--r-muted);line-height:1.55}}
    .jom-report-kicker{{font-size:12px;text-transform:uppercase;letter-spacing:.08em;font-weight:800;color:#253858;margin:0}}
    .jom-report-grid{{display:grid;grid-template-columns:repeat(auto-fit,minmax(190px,1fr));gap:14px;margin:24px 0}}
    .jom-report-card{{border:1px solid var(--r-border);border-radius:14px;padding:16px;background:#fbfcff}}
    .jom-report-card span{{display:block;text-transform:uppercase;letter-spacing:.06em;font-size:12px;font-weight:800;color:#44546f}}
    .jom-report-card strong{{display:block;font-size:30px;color:var(--r-blue);margin:8px 0}}
    .jom-report-card small{{color:var(--r-muted)}}
    .jom-report-section{{border-top:1px solid var(--r-border);padding-top:22px;margin-top:22px}}
    .jom-report-list{{line-height:1.65;color:var(--r-ink)}}
    .jom-report-table-wrap{{overflow:auto;border:1px solid var(--r-border);border-radius:14px}}
    .jom-report-table{{width:100%;border-collapse:collapse;font-size:14px}} .jom-report-table th,.jom-report-table td{{padding:12px;border-bottom:1px solid var(--r-border);text-align:left;vertical-align:top}}
    .jom-report-table th{{background:#f7f8f9;text-transform:uppercase;letter-spacing:.05em;font-size:12px;color:#44546f}}
    .jom-report-pill{{display:inline-flex;border-radius:999px;background:#deebff;color:#0052cc;padding:4px 8px;font-weight:800;white-space:nowrap}}
    details{{margin-top:24px}} pre{{white-space:pre-wrap;background:#f7f8f9;border:1px solid var(--r-border);border-radius:12px;padding:18px;overflow:auto}}
  </style>
</head>
<body>
  <main>
    {body}
    <p class='jom-report-muted'>Generated by Jira Observation Monitor at {_html_escape(generated)}.</p>
    <details><summary>Technical JSON</summary><pre>{raw_json}</pre></details>
  </main>
</body>
</html>"""
