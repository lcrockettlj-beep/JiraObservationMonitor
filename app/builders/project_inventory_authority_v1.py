...
def timestamp_semantics(): return {'generated_at_utc':'project inventory authority contract generation time','source_collection_time':'not represented','authority_change_time':'not represented'}
...
def unavailable(reason,sites):
 return {'schema':'jom-project-inventory-authority-v1','generated_at_utc':now_utc(),'timestamp_semantics':timestamp_semantics(),'status':'unavailable','read_only':True,'authority':{'safe_to_publish_project_inventory':False,'reason':reason,'fabricated_projects':False},'capabilities':capabilities(False),'summary':{'monitored_site_count':len(sites),'successful_site_count':0,'failed_site_count':len(sites),'visible_project_count':None},'sites':[],'projects':[]}
