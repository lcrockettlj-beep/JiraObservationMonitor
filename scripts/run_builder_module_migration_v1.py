...
STATUS = ROOT / "reports" / "builder_module_migration_v1_status.json"

MIGRATIONS = [
    ("scripts/build_named_access_recovery_plan.py", "app/access/named_access_recovery_plan.py", "named_access_recovery_plan"),
    ("scripts/build_named_access_reconciliation.py", "app/access/named_access_reconciliation.py", "named_access_reconciliation"),
    ("scripts/reconcile_named_access_truth_v2.py", "app/access/reconcile_named_access_truth_v2.py", "reconcile_named_access_truth_v2"),
]
...
def main() -> int:
    ...
    backup_root = ROOT / "backups" / f"builder_module_migration_v1_{stamp}"
    ...
    if not init.exists():
        init.write_text('"""JOM access package."""\n', encoding="utf-8")
    ...
    reliability = read_json(reliability_json) or {}
    ...
    rollback = backup_root / "rollback_builder_module_migration_v1.ps1"
    lines = [
    ...
    lines.append('Write-Host "Builder module migration v1 rollback complete." -ForegroundColor Green')
    rollback.write_text("\n".join(lines) + "\n", encoding="utf-8")
    ...
    status = {
        "schema": "jom-builder-module-migration-v1-status",
        "generated_at_utc": now_utc(),
        "mode": "phase_1_lowest_risk_access_report_builders_with_wrappers",
