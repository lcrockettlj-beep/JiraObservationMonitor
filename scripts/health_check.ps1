...
# ============================================================
#
# Runs 12 health checks across the full JOM platform.
...
function Test-Warning {
    ...
    $script:Warnings++
    $script:Passed++   # warnings still count as overall pass
}
...
try {
    ...
    if ($task.State -eq "Ready" -and $info.LastTaskResult -eq 0) {
        ...
        Test-Result 8 "Scheduled task" $false "LastResult=$($info.LastTaskResult)" "Check scripts\scheduled_sync.log for errors"
    } else {
...
if ($exposed.Count -eq 0) {
    Test-Result 11 "Secrets protection" $true "All $($secrets.Count) secrets gitignored"
} else {
    Test-Result 11 "Secrets protection" $false "Not gitignored: $($exposed -join ', ')" "Add to .gitignore immediately"
}

# ------------------------------------------------------------
# Check 12: Governance pack present
# ------------------------------------------------------------
$governanceFiles = @(
...
if ($missingGov.Count -eq 0) {
    Test-Result 12 "Governance pack" $true "$($governanceFiles.Count) governance files present"
} else {
    Test-Result 12 "Governance pack" $false "Missing: $($missingGov.Count) of $($governanceFiles.Count)" "Check $($missingGov[0])"
}
