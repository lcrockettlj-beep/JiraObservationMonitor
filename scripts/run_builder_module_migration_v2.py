...
STATUS = ROOT / "reports" / "builder_module_migration_v2_status.json"

MIGRATIONS = [
    ("scripts/build_named_access_truth_v2.py", "app/access/named_access_truth_v2.py", "named_access_truth_v2"),
    ("scripts/build_user_footprint_source.py", "app/access/user_footprint_source.py", "user_footprint_source"),
]
...
def main() -> int:
    ...
    backup_root = ROOT / "backups" / f"builder_module_migration_v2_{stamp}"
    ...
    named_truth = read_json(ROOT / "static" / "data" / "named_access_truth_v2.json") or {}
    ...
    rollback = backup_root / "rollback_builder_module_migration_v2.ps1"
    ...
    lines.append('Write-Host "Builder module migration v2 rollback complete." -ForegroundColor Green')
    rollback.write_text("\n".join(lines) + "\n", encoding="utf-8")
    ...
    status = {
        "schema": "jom-builder-module-migration-v2-status",
        "generated_at_utc": now_utc(),
        "mode": "phase_2_access_truth_builders_with_wrappers",
        ...
        "named_access_truth_summary_keys": sorted(list(named_truth.keys()))[:80] if isinstance(named_truth, dict) else [],
        "user_footprint_summary_keys": sorted(list(footprint.keys()))[:80] if isinstance(footprint, dict) else [],
