...
STATUS = ROOT / "reports" / "runtime_truth_builder_migration_v1_status.json"

MIGRATIONS = [
    ("scripts/build_admin_truth_layer_v2.py", "app/builders/admin_truth_layer_v2.py", "admin_truth_layer_v2"),
    ("scripts/build_estate_product_access.py", "app/builders/estate_product_access.py", "estate_product_access"),
]
...
def wrapper_for(module_path: Path, module_name: str) -> tuple[str, str]:
    text = module_path.read_text(encoding="utf-8", errors="replace")
    if "def main" in text:
        return "import_main", IMPORT_MAIN_WRAPPER.format(module_name=module_name)
    return "runpy", RUNPY_WRAPPER.format(module_name=module_name)
...
def main() -> int:
    ...
    backup_root = ROOT / "backups" / f"runtime_truth_builder_migration_v1_{stamp}"
    ...
    (ROOT / "app" / "builders").mkdir(parents=True, exist_ok=True)
    init = ROOT / "app" / "builders" / "__init__.py"
    if not init.exists():
        init.write_text('"""JOM builder modules."""\n', encoding="utf-8")
    ...
    for src_rel, dst_rel, module_name in MIGRATIONS:
        ...
        try:
            ...
            if f"from app.builders.{module_name} import main" in src_text or f'runpy.run_module("app.builders.{module_name}"' in src_text:
            ...
            wrapper_style, wrapper_text = wrapper_for(dst, module_name)
    ...
    rollback = backup_root / "rollback_runtime_truth_builder_migration_v1.ps1"
    lines = [
    ...
    lines.append('Write-Host "Runtime truth builder migration v1 rollback complete." -ForegroundColor Green')
    rollback.write_text("\n".join(lines) + "\n", encoding="utf-8")
    ...
    status = {
        "schema": "jom-runtime-truth-builder-migration-v1-status",
        "generated_at_utc": now_utc(),
        "mode": "phase_1_reportable_truth_builders_with_wrappers",
