...
Intelligence-aware runtime source adapter.

Preference order:
1. latest_run_intelligence.json
2. latest_run_alerted.json
3. latest_run_admin_enriched.json
4. latest_run.json
5. latest_run_safe_partial.json
6. pretty/report variants
"""
...
RUNTIME_CANDIDATE_GROUPS = [
    ["latest_run_intelligence.json", "reports/latest_run_intelligence.json"],
    ...
    ["latest_run_intelligence_pretty.json", "reports/latest_run_intelligence_pretty.json"],
...
def _managed_user_rows(users: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    return [u for u in users if str(u.get('accountType', '')).strip().lower() == 'atlassian']
...
def _build_estate(payload: Dict[str, Any], sites: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    if admin_summary:
        ...
        estate['human_user_count'] = admin_summary.get('human_user_count')
        estate['app_account_count'] = admin_summary.get('app_account_count')
...
def _normalise_payload(payload: Dict[str, Any], source_file: str) -> Dict[str, Any]:
    ...
    data['intelligence_summary'] = _safe_dict(payload.get('intelligence_summary'))
    data['intelligence_watchlist'] = _safe_list(payload.get('intelligence_watchlist'))
    ...
    if admin_summary:
        data['users_export_breakdown'] = [
            ...
            {'label': 'Managed-like users', 'value': admin_summary.get('managed_user_count', 0)},
            {'label': 'Human users', 'value': admin_summary.get('human_user_count', 0)},
            {'label': 'App accounts', 'value': admin_summary.get('app_account_count', 0)},
            ...
            {'label': 'Not in userbase', 'value': admin_summary.get('not_in_userbase_count', 0)},
        ]
    else:
