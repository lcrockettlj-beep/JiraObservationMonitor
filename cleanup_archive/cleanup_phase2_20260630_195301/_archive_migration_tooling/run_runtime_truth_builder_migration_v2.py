...
STATUS = ROOT / "reports" / "runtime_truth_builder_migration_v2_status.json"

MIGRATIONS = [
    ("scripts/refresh_admin_enriched_sources.py", "app/builders/admin_enriched_sources.py", "admin_enriched_sources"),
    ("scripts/refresh_product_access_sources.py", "app/builders/product_access_sources.py", "product_access_sources"),
    ("scripts/refresh_runtime_sources.py", "app/runtime/runtime_sources_refresh.py", "runtime_sources_refresh"),
]
...
def main() -> int:
    ...
    backup_root = ROOT / "backups" / f"runtime_truth_builder_migration_v2_{stamp}"
    ...
    for init in [ROOT / "app" / "builders" / "__init__.py", ROOT / "app" / "runtime" / "__init__.py"]:
        if not init.exists():
            init.write_text('"""JOM package."""\n', encoding="utf-8")
    ...
    rollback = backup_root / "rollback_runtime_truth_builder_migration_v2.ps1"
    lines = [
    ...
    lines.append('Write-Host "Runtime truth builder migration v2 rollback complete." -ForegroundColor Green')
    rollback.write_text("\n".join(lines) + "\n", encoding="utf-8")
    ...
    status = {
        "schema": "jom-runtime-truth-builder-migration-v2-status",
        "generated_at_utc": now_utc(),
        "mode": "phase_2_source_refresh_builders_with_wrappers",
